...
export default {
  ...
  methods: {
    ...
    // 单击获取新的图片验证码
    /* 这里使用e的话后面调用方法无法获取e 所以无法调用应该使用ref
      getCaptcha (e) {
        e.target.src = 'http://localhost:4000/captcha?time=' + Date.now()
      }
    */
    getCaptcha () {
      // 通过动态添加当前时间发送请求（因为如果请求地址不变不会再次发送请求获取新值）
      this.$refs.captcha.src = 'http://localhost:4000/captcha?time=' + Date.now()
    }
  },
  computed: {
    // 使用添加类的方式，通过计算属性匹配正则让获取验证码颜色高亮
    rightPhone () {
